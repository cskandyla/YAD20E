...
void InfoSurface::ShowAt(SDL_Texture *screen,int x,int y)
{
	SDL_Rect temp;
...
void InfoSurface::ShowAt(SDL_Texture *screen,int x,int y,int start,int end)
{
	...
	SDL_Rect temp;
	//I would like start to be at starty
	for(int i=start; i<=end; i++)
	{
		temp.x=startx;//posx+startx;
		temp.y=starty+(i-start)*TTF_FontHeight(font);
		SDL_QueryTexture(surfaces[i],NULL,NULL,&temp.w,&temp.h);
		BaseRenderer::Instance()->RenderTexture(surfaces[i],background,NULL,&temp);
...
int InfoSurface::getMaxSize()
{
	int width=0;
	int height=0;
	SDL_QueryTexture(background,NULL,NULL,&width,&height);
	
	return height/TTF_FontHeight(font);
