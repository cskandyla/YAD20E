...
int main ( int argc, char* argv[] )
{
	...
	window=SDL_CreateWindow("YAD20E",SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT,0);
//	window=SDL_CreateWindow("YAD20E",SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_FULLSCREEN);
	if(window==NULL)
	{
	...
	TTF_Font *fsmall=TTF_OpenFont("Resources/Fonts/Alpha Romanie.ttf",18);
	TTF_Font *consolefont=TTF_OpenFont("Resources/Fonts/Alpha Romanie.ttf",20);
  
	TTF_Font *movesfont=TTF_OpenFont("Resources/Fonts/FantasqueSansMono-Regular.ttf",14);
	TTF_Font *black_magic_font =TTF_OpenFont("Resources/Fonts/black-magic.ttf",20);
	TTF_Font *logfont=TTF_OpenFont("Resources/Fonts/CNR.ttf",12);
	//SDL_mixer
...
//  SDL_SetRenderpDrawColor(renderer,0,0,0,255);
//  SDL_SetRenderTarget(renderer,Toolbox_Surface);
	SDL_RenderClear(renderer);
  
	//SDL_FillRect(Toolbox_Surface,NULL,0x009000);
	SDL_Rect logRect;
	...
	myLog.AddText(string("Welcome to Cellars n Cantrips pre-alpha v0.01"),192,192,192);
	myLog.AddText(string(""),192,192,192);
	myLog.AddText(string("Use the menu on the bottom left to interact"),192,192,192);
	myLog.AddText(string("with the game!"),192,192,192);
	myLog.AddText(string(""),192,192,192);
	myLog.AddText(string("Pressing the TAB key opens the console"),192,192,192);
	myLog.AddText(string("Use the console to switch character"),192,192,192);
	myLog.AddText(string("Type class(mage) to get the mage"),192,192,192);
	myLog.AddText(string("Type class(warrior) to get the warrior"),192,192,192);
	myLog.AddText(string("Type class(cleric) to get the cleric"),192,192,192);
	...
	cout<<"Lua State"<<lua_state<<endl;
	//GAME ENGINE
	GameEngine *GE=new GameEngine(lua_state);
	...
	//Labels and Dialogs  
	SimpleLabel label(KnowledgePool::Instance()->getCharacter(1)->getInfo()->getName(),fsmall);
	SDL_Rect dia_rect;
	dia_rect.x=0;
	dia_rect.w=512;
	dia_rect.h=128;
	dia_rect.y=SCREEN_HEIGHT-dia_rect.h;//0;
	//Arduin's portrait:

	//Ase's portrait:
	SDL_Texture *left_portrait=BaseRenderer::Instance()->LoadImage("Resources/UI/Portraits/Arduin.png");
	SDL_Texture *right_portrait=BaseRenderer::Instance()->LoadImage("Resources/UI/Portraits/Ase.png");
	CharacterDialog *dial=new CharacterDialog(left_portrait,right_portrait,dia_rect,logfont);
	...
	while (!done)
	{
		...
		//label.Draw(KnowledgePool::Instance()->getCharacter(1)->getPosition().x/TILE_SIZE,KnowledgePool::Instance()->getCharacter(1)->getPosition().y/TILE_SIZE,Main_Game_Surface);
