...
bool MapIndex::DoodadIndexUpdate(Doodad *doodad,int xpos,int ypos)
{
...
bool  MapIndex::Mark(int y,int x)
{
  ...
  if(themap->getCollisionTiles()[y][x]->getID()==0 || themap->getCollisionTiles()[y][x]->getID()==2 /*|| getCell(y,x)!=NULL*/)
    return true;
  return false;
...
{
  if( (themap->getCollisionTiles()[x][y]->getID()!=0 && themap->getCollisionTiles()[x][y]->getID()!=2)  /*&& !informationgrid[y][x]*/)
    {
        return true;
    }
